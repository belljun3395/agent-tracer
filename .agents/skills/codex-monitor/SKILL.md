...
Codex에서 최종 assistant 답변 텍스트를 `assistant.response`로 남겨야 한다면
이 skill 경로가 캐노니컬이다. native hook만으로는 final answer text를 얻을 수 없다.
...
6. 최종 답변 직전에 `monitor_assistant_response`로 assistant 응답을 기록.
   - Codex에서 final answer text를 남기는 유일한 canonical path다.
...
- 하지만 hook만으로는 `assistant.response` 본문을 캡처할 수 없다.
- 따라서 assistant 답변 텍스트가 필요한 작업은 `codex-monitor` skill을 primary path로 사용한다.
