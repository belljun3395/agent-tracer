...
2. `monitor_task_start` (`title`: `Codex - <workspace>`, `runtimeSource`: `"codex-skill"`)
3. `monitor_user_message` (`captureMode="raw"`, `phase="initial"`, `source="manual-mcp"`) — 사용자 요청 기록
4. `monitor_explore` — 코드베이스·레퍼런스 탐색
5. `monitor_save_context` (`lane="planning"`) — 계획 스냅샷
6. `monitor_terminal_command` + `monitor_tool_used` — 구현
7. `monitor_task_complete` 또는 `monitor_task_error`
