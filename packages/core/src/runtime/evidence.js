 ...
 */
export function listRuntimeCoverage(runtimeSource) {
    ...
    const items = profile.features.map((feature) => ({
        key: feature.id,
        label: feature.label,
 ...
 */
export function getRuntimeCoverageSummary(runtimeSource) {
    const adapterId = normalizeRuntimeAdapterId(runtimeSource);
    if (!adapterId) {
        return {
