# packages/langgraph-agents: Python LangGraph 에이전트 서비스

AI 잡 에이전트 3종(recipe-scan·title-suggestion·task-cleanup)을 실행하는 Python 사이드카.
...
- app은 agent별 슬라이스·runtime·shared를 조립하고, agent별 슬라이스는 runtime/shared만
  ...
  저장용 `costUsd` 환산은 temporal-worker가 소유한다.

## 계약 불변식

- 이 서비스는 **무상태**다. DB·OpenSearch에 직접 접속하지 않는다. 데이터 조회는 요청 본문의
  `toolCallback`으로 temporal-worker를 되불러서만 한다. 토큰이 곧 소유 스코프라 `userId`를 받지 않는다.
- 오류는 `error.subtype`으로 노출한다. 문자열 목록은 temporal-worker `retry.policy.ts`가 소유하니,
  서브타입 이름을 바꾸면 그쪽 화이트리스트와 함께 고쳐야 한다.
- 세 에이전트의 프롬프트·중간 스키마·agent-facing 도구 설명·그래프·내부 한도는 이 서비스가
  소유한다. temporal-worker는 실행 envelope와 데이터 capability만 전달하고 외부 DTO를 재검증한다.
