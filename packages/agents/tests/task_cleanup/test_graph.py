...
async def test_검토자가_읽은_후보와_빈_껍데기를_조율자가_함께_제안한다(
    ...
    narrate(
        "task-cleanup :: 검토자가 읽은 후보와 노출된 빈 껍데기를 조율자가 함께 제안한다",
        chat,
        res,
    )
...
async def test_선별자가_노출하지_않은_후보는_버린다(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    narrate("task-cleanup :: 선별자가 노출한 적 없는 후보 제안은 검증에서 버려진다", chat, res)
...
async def test_검토자가_읽지_않은_이벤트_후보는_제안으로_받지_않는다(
    ...
    narrate("task-cleanup :: 검토자가 읽지 않은 이벤트 후보는 조율자가 제안해도 버려진다", chat, res)
...
async def test_검토자가_읽었더니_이벤트가_없는_후보는_인용_없이도_받는다(
    ...
    narrate("task-cleanup :: 검토자가 읽었더니 이벤트가 없던 후보는 인용 없이도 제안으로 받는다", chat, res)
...
async def test_조율자가_재파견을_요청하면_후보를_한_번_더_열어보고_완주한다(
    ...
    narrate("task-cleanup :: 조율자가 재파견을 요청하면 후보를 한 번 더 열어보고 완주한다", chat, res)
