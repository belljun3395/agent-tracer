...
async def test_대화_발췌로_충분하면_도구를_부르지_않고_제목을_낸다(
    ...
    chat, res, ledger = await _run(monkeypatch, [_SUGGESTIONS])
    ...
    narrate("title-suggestion :: 대화 발췌만으로 도구 없이 제목을 낸다", chat, res)


async def test_현재_제목이_적절하면_빈_결과를_낸다(monkeypatch: pytest.MonkeyPatch) -> None:
    chat, res, _ledger = await _run(monkeypatch, [{"suggestions": []}])

    assert res.error is None and res.data == {"suggestions": []}
    narrate("title-suggestion :: 현재 제목이 이미 적절하면 빈 제안을 낸다", chat, res)
...
async def test_발췌가_부족하면_모델이_스스로_이벤트를_읽는다(
    ...
    chat, res, fake_ledger = await _run(monkeypatch, turns, ledger)
    ...
    narrate("title-suggestion :: 발췌가 부족하면 태스크 이벤트를 직접 읽는다", chat, res)
...
async def test_현재_제목을_되풀이한_후보는_한_번_수정한다(
    ...
    chat, res, _ledger = await _run(monkeypatch, [repeated, _SUGGESTIONS])
    ...
    narrate("title-suggestion :: 현재 제목을 되풀이한 후보는 한 번 수정한다", chat, res)
...
async def test_수정_후에도_후보가_유효하지_않으면_빈_결과를_낸다(
    ...
    chat, res, _ledger = await _run(monkeypatch, [invalid, invalid])

    assert res.error is None and res.data == {"suggestions": []}
    assert sum(step.eventKind == "validation.failed" for step in res.steps) == 2
    narrate("title-suggestion :: 수정 후에도 후보가 유효하지 않으면 빈 결과를 낸다", chat, res)


async def test_단가를_모르는_모델은_내부_예산을_우회하지_못한다(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    chat, res, _ledger = await _run(monkeypatch, [_SUGGESTIONS], model="claude-custom-alias")

    assert res.error is not None
    assert "cannot enforce its internal budget" in res.error.summary
    narrate("title-suggestion :: 단가를 모르는 모델은 내부 예산을 우회하지 못한다", chat, res)
