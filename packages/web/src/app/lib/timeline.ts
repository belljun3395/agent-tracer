...
export function buildTimelineLayout(events: readonly TimelineEventRecord[], zoom: number, nowMs: number = Date.now(), laneRowsOrLanes: readonly TimelineLaneRow[] | readonly TimelineLane[] = TIMELINE_LANES, options?: TimelineLayoutOptions): TimelineLayout {
    ...
    const adjusted = new Map<TimelineEventRecord, number>();
    ...
    const rowIndexMap = new Map<TimelineEventRecord, number>();
...
export function buildTimestampTicks(events: readonly TimelineEventRecord[], layout: TimelineLayout, nowMs: number): readonly TimestampTick[] {
    if (events.length === 0)
        return [];
...
export function buildTimelineRelations(events: readonly TimelineEventRecord[]): readonly TimelineRelation[] {
...
function resolveLaneKeyForEvent(event: TimelineEventRecord, laneRows: readonly TimelineLaneRow[]): string {
