...
interface KnowledgeItemRowProps {
    ...
    readonly workflowContentBySnapshotId: Record<string, WorkflowContentRecord>;
...
export function KnowledgeItemRow(props: KnowledgeItemRowProps): React.JSX.Element {
    const {
        ...
        loadingKey,
        failedKey,
        workflowContentBySnapshotId,
        onOpenSnapshotDetail,
        onSelectTask,
    } = props;

    const key = `snapshot:${item.snapshotId}`;
    const isExpanded = expandedKey === key;
    const content = workflowContentBySnapshotId[item.snapshotId];
        ...
        <div className="border-b border-[var(--border)] last:border-0">
            ...
            {isExpanded ? (
                loadingKey === key ? (
                    ...
                    <SnapshotDetail content={content} />
                ) : null
            ) : null}
        </div>
    );
