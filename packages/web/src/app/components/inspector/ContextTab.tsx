...
function metaString(metadata: TimelineEventRecord["metadata"], key: string): string | null {
    const value = metadata[key];
    return typeof value === "string" && value.length > 0 ? value : null;
}

function metaNumber(metadata: TimelineEventRecord["metadata"], key: string): number | null {
    const value = metadata[key];
...
function buildInstructionRow(event: TimelineEventRecord): InstructionRow {
...
function buildContextRow(event: TimelineEventRecord): ContextRow {
    return {
        event,
        title: event.title,
        body: event.body ?? null,
