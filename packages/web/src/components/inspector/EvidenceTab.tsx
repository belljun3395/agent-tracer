/**
 * Evidence 탭 — 태그 탐색, 파일 활동, 탐색 인사이트, 웹 조회, 파일 탐색, 멘션 검증 패널 뷰.
 */
...
import type {
  ...
  ExploredFileStat,
  FileActivityStat,
  FileMentionVerification,
  DirectoryMentionVerification,
  MentionedFileVerification,
  TagInsight,
  WebLookupStat
} from "../../lib/insights.js";

// ---------------------------------------------------------------------------
// SectionTitle helper (local, mirrors usage in TagExplorerCard)
// ---------------------------------------------------------------------------
...
function SectionTitle({
  ...
  return (
    <div className="flex items-start justify-between gap-3">
      <div>
        {eyebrow && (
          <div className="mb-1 text-[0.68rem] font-bold uppercase tracking-[0.1em] text-[var(--text-3)]">{eyebrow}</div>
        )}
        <strong className="text-[0.95rem] text-[var(--text-1)]">{title}</strong>
        {description && (
          <p className="mt-0.5 mb-0 text-[0.78rem] text-[var(--text-3)]">{description}</p>
        )}
      </div>
      {action}
    </div>
...
// ---------------------------------------------------------------------------
// FileMentionRow / DirectoryMentionRow / MentionedFilesVerificationCard
// ---------------------------------------------------------------------------
...
export function EvidenceTab({
  ...
  sortedFileActivity,
  workspacePath,
  isFileActivityExpanded,
  fileSortKey,
  explorationInsight,
  webLookups,
  sortedExploredFiles,
  isExploredFilesExpanded,
  explorationSortKey,
  mentionedVerifications,
  onSelectTag,
  onToggleFileActivity,
  onFileSortChange,
  onToggleExploredFiles,
  onExplorationSortChange
}: EvidenceTabProps): React.JSX.Element {
  return (
    <div className="panel-tab-inner flex flex-col gap-5 p-4">
      ...
      <ExploredFilesSection
        ...
        onToggle={onToggleExploredFiles}
        onSortChange={onExplorationSortChange}
      />
