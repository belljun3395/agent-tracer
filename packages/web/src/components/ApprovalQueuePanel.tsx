...
export function ApprovalQueuePanel({
  ...
  onSelectTask,
  onRefresh
}: ApprovalQueuePanelProps): React.JSX.Element {
  ...
  const queueTasks = useMemo(
    () => tasks.filter((task) => task.status === "waiting" || task.status === "errored"),
    [tasks]
  );
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setItems(
            filtered.map(({ task, observability }) => ({ task, observability }))
          );
          setRecentDecisions(
            filtered
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-label="Approval queue">
      <button
        ...
        <PanelCard className="max-h-[82vh] overflow-hidden rounded-[14px]">
          ...
          <div className="space-y-3 overflow-y-auto p-4">
            <label className="block">
              ...
              <input
                className="w-full rounded-[8px] border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2 text-[0.82rem]"
            ...
            {loading ? (
              <p className="text-[0.82rem] text-[var(--text-3)]">Loading approval queue…</p>
            ) : items.length === 0 ? (
              <p className="text-[0.82rem] text-[var(--text-3)]">No tasks currently require approval or bypass review.</p>
            ) : (
              items.map((item) => (
                <div key={item.task.id} className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-4">
                  ...
                  <label className="mt-3 block">
                    ...
                    <textarea
                      className="min-h-[72px] w-full rounded-[8px] border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2 text-[0.8rem] leading-6"
                      onChange={(event) => setNotesByTaskId((current) => ({
                        ...current,
                        [item.task.id]: event.target.value
                      }))}
                      placeholder="Optional decision note"
                      value={notesByTaskId[item.task.id] ?? ""}
                    />
                  </label>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {item.observability.ruleEnforcement.activeState === "approval_required" && (
                      <>
                        <Button
                          disabled={actingTaskId === item.task.id}
                          onClick={() => void handleAction(item, "approved")}
                          size="sm"
                        >
                          Approve
                        </Button>
                        <Button
                          disabled={actingTaskId === item.task.id}
                          onClick={() => void handleAction(item, "rejected")}
                          size="sm"
                          variant="destructive"
                        >
                          Reject
                    ...
                    <Button
                      disabled={actingTaskId === item.task.id}
                      onClick={() => void handleAction(item, "bypassed")}
                      size="sm"
                      variant="ghost"
                    >
                      Bypass
              ...
              <div className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-4">
                ...
                <div className="flex flex-col gap-2">
                  {recentDecisions.map((decision) => (
                    <div key={decision.id} className="rounded-[10px] border border-[var(--border)] bg-[var(--bg)] px-3 py-2">
                      <div className="flex flex-wrap items-center gap-2">
