...
function WorkflowContentDetail({ content }: { readonly content: WorkflowContentRecord }): React.JSX.Element {
...
export function WorkflowLibraryPanel({ onSelectTask, onClose }: WorkflowLibraryPanelProps): React.JSX.Element {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-[2px]" onClick={onClose}>
      <div
        className="relative mt-12 flex max-h-[80vh] w-full max-w-3xl flex-col overflow-hidden rounded-[16px] border border-[var(--border)] bg-[var(--surface)] shadow-[0_24px_64px_rgba(0,0,0,0.18)]"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {isLoading ? (
            ...
            items.map((item) => (
              <div key={item.taskId} className="border-b border-[var(--border)] last:border-0">
                <div className="flex items-start gap-3 px-4 py-3.5 transition-colors hover:bg-[var(--surface-2)]">
                  ...
                  <div className="flex shrink-0 flex-col gap-2">
                    <button
                      type="button"
                      className="rounded-[7px] border border-[var(--border)] bg-[var(--surface)] px-2.5 py-1 text-[0.72rem] font-semibold text-[var(--text-2)] transition-colors hover:text-[var(--text-1)]"
                ...
                {expandedTaskId === item.taskId && (
                  <>
                    ...
                    ) : failedTaskId === item.taskId ? (
                      <div className="border-t border-[var(--border)] bg-[var(--surface-2)] px-4 py-3 text-[0.76rem] text-[var(--danger)]">
                        Failed to load workflow content.
                      </div>
                    ) : contentByTaskId[item.taskId] ? (
                      <WorkflowContentDetail content={contentByTaskId[item.taskId]} />
                    ) : null}
                  </>
                )}
              </div>
