...
export function TaskEvaluatePanel({ taskId, taskTitle, taskTimeline, evaluation, isSaving, isSaved, onSave }: TaskEvaluatePanelProps): React.JSX.Element {
    ...
    return (<div className="flex flex-col gap-4 rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-4">
      <div className="flex items-center gap-2">
        ...
        {evaluation && (<span className={cn("rounded-full px-2 py-0.5 text-[0.68rem] font-semibold", evaluation.rating === "good"
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <input className="rounded-[6px] border border-[var(--border)] bg-[var(--surface-2)] px-2.5 py-1.5 text-[0.8rem] text-[var(--text-1)] outline-none placeholder:text-[var(--text-3)] focus:border-[var(--accent)]" placeholder="What kind of task was this? e.g. TypeScript type error fix" value={useCase} onChange={(event) => setUseCase(event.target.value)}/>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <textarea className={textareaClass} placeholder="Summarize what this task resolved." rows={3} value={outcomeNote} onChange={(event) => setOutcomeNote(event.target.value)}/>
      </div>

      <div className="flex flex-col gap-1.5">
        <span className={labelClass}>What worked</span>
        <textarea className={textareaClass} placeholder="Describe what worked well." rows={3} value={approachNote} onChange={(event) => setApproachNote(event.target.value)}/>
      </div>

      <div className="flex flex-col gap-1.5">
        <span className={labelClass}>Reuse when</span>
        <textarea className={textareaClass} placeholder="Describe when this workflow should be reused." rows={2} value={reuseWhen} onChange={(event) => setReuseWhen(event.target.value)}/>
      </div>

      <div className="flex flex-col gap-1.5">
        <span className={labelClass}>Watch out</span>
        <textarea className={textareaClass} placeholder="List any watchouts for future runs." rows={2} value={watchouts} onChange={(event) => setWatchouts(event.target.value)}/>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-[12px] border border-[var(--border)] bg-[var(--surface-2)] p-4">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className={cn("rounded-full px-2.5 py-1 text-[0.68rem] font-semibold", workflowContentState === "Edited"
            ? "bg-[color-mix(in_srgb,var(--accent)_12%,transparent)] text-[var(--accent)]"
            : "bg-[var(--surface)] text-[var(--text-3)]")}>
              {workflowContentState}
            </span>
            <button type="button" className={cn("rounded-[7px] border px-2.5 py-1 text-[0.74rem] font-semibold transition-colors", workflowContentMode === "preview"
            ? "border-[var(--accent)] bg-[color-mix(in_srgb,var(--accent)_10%,transparent)] text-[var(--accent)]"
            : "border-[var(--border)] bg-[var(--surface)] text-[var(--text-2)] hover:text-[var(--text-1)]")} onClick={() => setWorkflowContentMode("preview")}>
              Preview
            </button>
            <button type="button" className={cn("rounded-[7px] border px-2.5 py-1 text-[0.74rem] font-semibold transition-colors", workflowContentMode === "edit"
            ? "border-[var(--accent)] bg-[color-mix(in_srgb,var(--accent)_10%,transparent)] text-[var(--accent)]"
            : "border-[var(--border)] bg-[var(--surface)] text-[var(--text-2)] hover:text-[var(--text-1)]")} onClick={() => setWorkflowContentMode("edit")}>
              Edit fields
            </button>
            <button type="button" className="rounded-[7px] border border-[var(--border)] bg-[var(--surface)] px-3 py-1.5 text-[0.74rem] font-semibold text-[var(--text-2)] transition-colors hover:text-[var(--text-1)]" onClick={handleRegenerateWorkflowContent}>
              Regenerate
            </button>
          </div>
        </div>
        ...
        {workflowContentMode === "preview" ? (<div className="flex flex-col gap-3">
            ...
            <div className={snapshotFieldClass}>
              ...
              <textarea className={textareaClass} rows={3} value={workflowSnapshotDraft.objective} onChange={(event) => updateSnapshotField("objective", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Original request</span>
              <textarea className={textareaClass} rows={4} value={workflowSnapshotDraft.originalRequest} onChange={(event) => updateSnapshotField("originalRequest", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Outcome summary</span>
              <textarea className={textareaClass} rows={4} value={workflowSnapshotDraft.outcomeSummary} onChange={(event) => updateSnapshotField("outcomeSummary", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Approach summary</span>
              <textarea className={textareaClass} rows={4} value={workflowSnapshotDraft.approachSummary} onChange={(event) => updateSnapshotField("approachSummary", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Reuse when</span>
              <textarea className={textareaClass} rows={3} value={workflowSnapshotDraft.reuseWhen} onChange={(event) => updateSnapshotField("reuseWhen", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Verification summary</span>
              <textarea className={textareaClass} rows={3} value={workflowSnapshotDraft.verificationSummary} onChange={(event) => updateSnapshotField("verificationSummary", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Key decisions</span>
              <textarea className={textareaClass} placeholder="One item per line" rows={5} value={workflowSnapshotDraft.keyDecisions} onChange={(event) => updateSnapshotField("keyDecisions", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Next steps</span>
              <textarea className={textareaClass} placeholder="One item per line" rows={5} value={workflowSnapshotDraft.nextSteps} onChange={(event) => updateSnapshotField("nextSteps", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Watch items</span>
              <textarea className={textareaClass} placeholder="One item per line" rows={4} value={workflowSnapshotDraft.watchItems} onChange={(event) => updateSnapshotField("watchItems", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Key files</span>
              <textarea className={textareaClass} placeholder="One item per line" rows={4} value={workflowSnapshotDraft.keyFiles} onChange={(event) => updateSnapshotField("keyFiles", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Modified files</span>
              <textarea className={textareaClass} placeholder="One item per line" rows={4} value={workflowSnapshotDraft.modifiedFiles} onChange={(event) => updateSnapshotField("modifiedFiles", event.target.value)}/>
            </div>
            <div className={snapshotFieldClass}>
              <span className={labelClass}>Search text</span>
              <textarea className={cn(textareaClass, "font-mono text-[0.78rem]")} rows={5} value={workflowSnapshotDraft.searchText} onChange={(event) => updateSnapshotField("searchText", event.target.value)}/>
            </div>

            <div className="flex flex-col gap-1.5">
              <span className={labelClass}>Workflow context</span>
              <textarea className={cn(textareaClass, "min-h-[28rem] font-mono text-[0.78rem] leading-7")} value={workflowContextDraft} onChange={(event) => {
