...
/**
 * A contiguous slice of a task's timeline bounded by user.message events.
 * Multi-turn tasks produce one segment per user prompt (plus an optional
 * prelude segment for events that happened before any prompt, such as task.start).
 */
 ...
 */
export function segmentEventsByTurn(events: readonly TimelineEventRecord[]): readonly TurnSegment[] {
    if (events.length === 0) {
        return [];
    }

    const boundaryIndices = events
        .map((event, index) => (event.kind === "user.message" ? index : -1))
    ...
    for (let i = 0; i < boundaryIndices.length; i += 1) {
        ...
        segments.push({
            ...
            endAt,
            requestPreview: buildRequestPreview(userMessage),
            events: turnEvents,
        });
    }
...
function buildRequestPreview(event: TimelineEventRecord): string | null {
...
export function filterEventsByTurnRange(
    ...
    const collected: TimelineEventRecord[] = [];
