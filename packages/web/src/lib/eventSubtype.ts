...
export function resolveEventSubtype(event: Pick<TimelineEvent, "lane" | "metadata" | "title">): EventSubtype | null {
    ...
    const icon = subtypeIcon(legacyKey);
    const entityType = extractMetadataString(event.metadata, "entityType");
    const entityName = extractMetadataString(event.metadata, "entityName");
    return {
        key: legacyKey,
...
export function countLaneSubtypes(events: readonly TimelineEvent[], lane: ExpandableTimelineLane): number {
...
export function buildDisplayLaneRows(events: readonly TimelineEvent[], activeLanes: readonly TimelineLane[], expandedLanes: ReadonlySet<ExpandableTimelineLane>): readonly TimelineLaneRow[] {
...
export function resolveTimelineRowKey(event: TimelineEvent, expandedLanes: ReadonlySet<ExpandableTimelineLane>): string {
...
function inferLegacySubtypeKey(event: Pick<TimelineEvent, "lane" | "metadata" | "title">): EventSubtypeKey | null {
    ...
    if (event.lane === "exploration") {
        ...
        if (normalized.includes("grep") || normalized.includes("search"))
            return "grep_code";
        if (normalized.includes("list") || normalized.includes("tree"))
            return "list_files";
    ...
    if (event.lane === "implementation") {
        ...
        if (normalized.includes("create") || normalized.includes("write"))
            return "create_file";
        if (normalized.includes("edit") || normalized.includes("modify") || normalized.includes("update"))
            return "modify_file";
        if (normalized.includes("test"))
            return "run_test";
        if (normalized.includes("build"))
            return "run_build";
        if (normalized.includes("lint") || normalized.includes("format"))
            return "run_lint";
        if (normalized.includes("verify") || normalized.includes("check"))
            return "verify";
        if (normalized.includes("rule") || normalized.includes("policy"))
            return "rule_check";
