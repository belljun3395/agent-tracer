/** 앱 껍데기와 상태 표시에 쓰는 24x24 단일 stroke SVG 아이콘이다. */

import type { IconProps } from "~web/shared/ui/icons/icon-props.js";
export function ActivityIcon({ size = 13, className }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
...
/** row 단위 "edit" 액션에 쓰는 밑줄 있는 연필 글리프. */

export function SparkleIcon({ spinning, size = 12 }: { readonly spinning: boolean; readonly size?: number }) {
  return (
    <svg
      width={size}
      height={size}
...
export function HamburgerIcon({ size = 14, className }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
