/**
 * Smoke tests for useTaskStore (task slice).
 *
 * Verifies default initial state and basic state mutations via
 * dispatchTaskAction. Integration with async API calls is not tested here
 * (those belong to integration tests).
 */

import { beforeEach, describe, expect, it } from "vitest";
import { _taskStore } from "./useTaskStore.js";
...
beforeEach(() => {
  // Reset to a clean initial state by re-applying a full reset through the
  // known actions. We can't call _taskStore.setState directly on the inner
  // taskState without exposing internals, so we drive via dispatch.
...
describe("useTaskStore — PATCH_TASK_DISPLAY_TITLE_CACHE", () => {
  ...
  it("returns the same state reference when nothing changed (identity equality)", () => {
    ...
    // Dispatching identical data should not produce a new object
    _taskStore.getState().dispatchTaskAction({
      type: "PATCH_TASK_DISPLAY_TITLE_CACHE",
      taskId: "task-1",
      title: "Same Title",
