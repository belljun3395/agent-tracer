...
function mergeTimeline(prev: readonly TimelineEvent[], next: readonly TimelineEvent[]): readonly TimelineEvent[] {
    ...
    const unchanged = merged.length === prev.length && merged.every((e, i) => e === prev[i]);
    return unchanged ? prev : merged;
}
export function mergeTaskDetail(prev: TaskDetailResponse, detail: TaskDetailResponse): TaskDetailResponse {
    if (prev.task.id !== detail.task.id)
        return detail;
...
export interface TaskState {
    ...
    readonly taskDisplayTitleCache: Readonly<Record<string, {
...
export type TaskAction = {
    ...
    type: "SET_DELETING_TASK_ID";
    taskId: string | null;
} | {
    type: "SET_DELETE_ERROR_TASK_ID";
    taskId: string | null;
} | {
    type: "SET_NOW_MS";
    nowMs: number;
    ...
    type: "SET_TASK_TITLE_DRAFT";
    draft: string;
} | {
    type: "SET_TASK_TITLE_ERROR";
    error: string | null;
} | {
    type: "SET_SAVING_TASK_TITLE";
    isSaving: boolean;
} | {
    type: "SET_UPDATING_TASK_STATUS";
    isUpdating: boolean;
} | {
...
export interface TaskStoreSlice {
    ...
    handleTaskTitleSubmit: (event: React.SyntheticEvent<HTMLFormElement>, nextTitle: string) => Promise<void>;
}
...
export const _taskStore = create<TaskStoreSlice>((set, get) => {
    ...
    async function refreshTaskDetail(taskId: string): Promise<void> {
        try {
            ...
            const status = err instanceof Error ? (err as Error & {
    ...
    async function handleTaskStatusChange(status: MonitoringTask["status"]): Promise<void> {
        ...
        try {
            ...
            dispatch({
                type: "SET_TASK_DETAIL",
                detail: getTaskState().taskDetail?.task.id === updatedTask.id
    ...
    async function handleTaskTitleSubmit(event: React.SyntheticEvent<HTMLFormElement>, nextTitle: string): Promise<void> {
