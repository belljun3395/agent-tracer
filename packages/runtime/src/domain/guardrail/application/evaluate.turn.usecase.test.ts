...
let source = new InMemoryRuleSource();
let usecase = new EvaluateTurnUsecase(source);

beforeEach(() => {
    source = new InMemoryRuleSource();
    usecase = new EvaluateTurnUsecase(source);
});
...
describe("EvaluateTurnUsecase", () => {
    it("근거 입력 이후 기대 명령이 실행되면 이행으로 본다", () => {
        ...
        expect(result.verdicts[0]?.status).toBe("satisfied");
    ...
    it("기대 명령이 없으면 턴을 붙잡는다", () => {
        ...
        expect(result.verdicts[0]?.status).toBe("open");
    ...
    it("근거 입력이 최근 이벤트 버퍼 밖이면 미이행이라 단언하지 않는다", () => {
        const events = [event("later", KIND.userMessage, "다른 얘기"), lintCall("e1")];

        const result = usecase.execute(events, [rule()], "task-1");

        expect(result.verdicts[0]?.status).toBe("unknown");
    ...
    it("막은 규칙은 알렸다고 서버에 남긴다", () => {
        usecase.execute([anchorEvent()], [rule()], "task-1");

        expect(source.nudged).toEqual(["rule-1"]);
    });

    it("상한을 넘긴 판정은 더는 막지 않는다", () => {
        const escalated = rule({escalated: true});

        expect(usecase.execute([anchorEvent()], [escalated], "task-1").blocking).toEqual([]);
    });
});
