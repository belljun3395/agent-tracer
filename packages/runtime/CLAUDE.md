...
- `recipe` `hint`: 레시피가 있다는 사실만 알리는 짧은 넛지와 본문 조립, 컨텍스트 주입 텍스트
  조립. 레시피 목록은 프롬프트에 싣지 않는다 — 관련성 판단은 에이전트가 `search_recipes`로
  질의어를 던져 얻은 결정 수준 정보(제목·의도·설명·점수)로 내리고, 적용할 레시피의 전체
  본문은 `get_recipe`로 연다.
...
- `daemon/`: 조립 근원과 제어 화면. `daemon/port/`가 훅과 데몬 사이 소켓 계약을
  ...
  MCP 브리지 전용 메시지(레시피 조회·레시피 검색·성과 보고·스캔 요청·제목 갱신)를 얹어 쓰며
  `daemon.socket.port.ts`의 `parseDaemonRequest`가 못 찾은 타입을 이쪽에 넘긴다.
  레시피 조회가 성공하면 그 시점에 데몬이 `recipeInjected` 이벤트를 내어 적용 이력을 연다.

MCP 도구가 묻는 "지금 태스크가 뭔가"는 추정하지 않는다. Claude Code가 세션마다 띄우는
MCP 서버 프로세스의 환경변수 `CLAUDE_CODE_SESSION_ID`에 그 서버가 딸린 세션의 식별자가
들어 있고(`config/env.ts`의 `resolveClaudeSessionId`), 브리지가 그것을 소켓 요청에 실어
데몬이 바인딩을 키로 정확히 찾는다. 못 찾으면 `unknown_session`으로 실패하고 가장 최근
