...
```mermaid
flowchart LR
    CC["코딩 에이전트"] -->|훅 이벤트| H["훅 엔트리<br/><small>얇은 어댑터</small>"]
    CC -->|도구 호출| M["MCP 서버<br/><small>얇은 어댑터</small>"]
    H -->|유닉스 소켓| D["데몬"]
    M -->|유닉스 소켓| D
    D --> SP[("스풀<br/><small>~/.agent-tracer</small>")]
    SP -->|배치 전송| S["서버"]
    SP -.->|영구 거부| DL[("데드레터")]
    D -->|규칙 판정·힌트| H
    D --> CTRL["제어 화면 :3848"]
...
훅은 이벤트를 밀어 넣는 통로이고 MCP 서버는 에이전트가 스스로 판단해 부르는 반대 방향
통로다. 둘 다 얇게 두고 처리는 데몬 소켓에 위임한다. 서버가 죽어 있어도 스풀이 받아 두므로
수집이 사용자의 작업을 붙잡지 않는다.
