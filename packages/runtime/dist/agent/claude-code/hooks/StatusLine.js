...
// src/agent/claude-code/payload/status.payload.ts
function readStatusLine(raw) {
  ...
  const snapshot = {
    ...
    ...inputTokens !== void 0 ? { contextWindowTotalTokens: inputTokens + (outputTokens ?? 0) } : {},
    ...
    ...version !== void 0 ? { sessionVersion: version } : {}
  };
  return {
    ok: true,
    value: {
      sessionId,
      snapshot,
      hasTelemetry: usedPct !== void 0 || Object.keys(rateLimits).length > 0,
      usedPct,
      fiveHourPct,
      costUsd: cost
    }
...
// src/domain/ingest/model/lifecycle.event.model.ts
function contextSnapshotEvent(target, snapshot) {
  ...
  return {
    ...
    ...turnOf(target),
    lane: LANE.telemetry,
    title: usedPct !== void 0 ? `Context ${Math.round(usedPct)}% used` : "Context snapshot",
    metadata
  };
}

// src/agent/claude-code/hooks/StatusLine.ts
...
`);
    ...
    const target = await ensureClaudeSession(payload.sessionId, void 0, { resume: false });
...
function formatStatusText(payload) {
  const parts = [];
  if (payload.usedPct !== void 0) parts.push(`ctx ${Math.round(payload.usedPct)}%`);
  if (payload.fiveHourPct !== void 0) parts.push(`5h ${Math.round(payload.fiveHourPct)}%`);
  if (payload.costUsd !== void 0 && payload.costUsd > 0) parts.push(`$${payload.costUsd.toFixed(3)}`);
  return parts.length > 0 ? `[monitor] ${parts.join(" \xB7 ")}` : "";
}
