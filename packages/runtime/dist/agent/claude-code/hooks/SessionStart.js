...
var SESSION_START_SOURCE = {
  startup: "startup",
  resume: "resume",
  clear: "clear",
  compact: "compact"
};
function readSessionStart(raw) {
...
function onSessionClear(hook, input) {
...
function clearClaudeSession(runtimeSessionId) {
...
function contextEvent(target, title, body, metadata) {
  return {
    kind: KIND.contextSaved,
    taskId: target.taskId,
    sessionId: target.sessionId,
...
function sessionTriggerEvent(target, trigger) {
...
// src/agent/claude-code/hooks/SessionStart.ts
await runHook("SessionStart", {
  parse: readSessionStart,
  handler: async (payload) => {
    const source = payload.source.toLowerCase();
    const target = source === SESSION_START_SOURCE.clear ? await clearClaudeSession(payload.sessionId) : await ensureClaudeSession(payload.sessionId, void 0, { transcriptPath: payload.transcriptPath });
