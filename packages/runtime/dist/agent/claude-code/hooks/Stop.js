...
function resolveTranscriptCursorDir(env = process.env) {
  return path11.join(resolveAgentTracerPaths(env).cacheDir, "claude-transcript-cursors");
}
...
function cursorPath(sourceSessionId, cursorDir) {
  ...
  return path11.join(cursorDir, `${key}.json`);
}
function hashFileHead(transcriptPath, length) {
  ...
  try {
    ...
    const bytesRead = fs10.readSync(fd, buffer, 0, length, 0);
...
function toCommentaryEvents(entries, sourceSessionId, target) {
