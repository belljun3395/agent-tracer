...
// src/agent/claude-code/payload/turn.payload.ts
function readStop(raw) {
...
function parseDaemonGuardrailResponse(value) {
  return isRecord(value) && Array.isArray(value["verdicts"]) ? { verdicts: value["verdicts"] } : null;
}
...
// src/daemon/ipc/hook.client.ts
var GUARDRAIL_TIMEOUT_MS = 1e3;
var ASSISTANT_TEXT_MAX = 8e3;
...
async function queryDaemonGuardrail(taskId, sessionId, candidateAssistantText) {
  if (!taskId) return [];
  ...
  try {
    return await requestDaemon(
      paths.socketPath,
      {
        ...
        ...candidateAssistantText ? { candidateAssistantText: candidateAssistantText.slice(0, ASSISTANT_TEXT_MAX) } : {}
      },
...
function deterministicUlid(parts) {
...
function onSessionEnd(hook, input) {
...
// src/agent/claude-code/transcript/transcript.wire.ts
var TRANSCRIPT_ENTRY_TYPE = {
  assistant: "assistant",
  user: "user"
};
var TRANSCRIPT_MESSAGE_ROLE = {
  assistant: "assistant",
  user: "user"
};
var TRANSCRIPT_BLOCK_TYPE = {
  text: "text",
  thinking: "thinking"
};
var TRANSCRIPT_STOP_REASON = {
  toolUse: "tool_use"
};
...
function readTranscriptEntries(transcriptPath, startOffset, fileSize) {
  ...
  try {
    ...
    const bytesRead = fs11.readSync(fd, buffer, 0, buffer.length, readStart);
...
function findResumedSessionId(transcriptPath, currentSessionId, maxLines = RESUME_SCAN_MAX_LINES) {
  if (!transcriptPath || !fs12.existsSync(transcriptPath)) return Promise.resolve(void 0);
  return new Promise((resolve2) => {
    let resumedFrom;
    let lineCount = 0;
    let settled = false;
    ...
    const stream = fs12.createReadStream(transcriptPath, { encoding: "utf8" });
...
function saveTranscriptCursor(sourceSessionId, cursor, cursorDir) {
  const current = loadTranscriptCursor(sourceSessionId, cursorDir);
  if (current?.transcriptPath === cursor.transcriptPath && current.fileSize >= cursor.fileSize && current.byteOffset > cursor.byteOffset) {
    try {
...
function resolveTranscriptStartOffset(cursor, transcriptPath, fileSize) {
...
function createTranscriptCursor(transcriptPath, byteOffset, fileSize) {
...
function isTranscriptCursor(value) {
  ...
  return typeof value["transcriptPath"] === "string" && isOffset(value["byteOffset"]) && isOffset(value["fileSize"]) && isOffset(value["headLength"]) && typeof value["headHash"] === "string";
}
...
function hashFileHead(transcriptPath, length) {
  ...
  try {
    ...
    const bytesRead = fs13.readSync(fd, buffer, 0, length, 0);
...
// src/agent/claude-code/transcript/transcript.usage.ts
...
// src/domain/ingest/model/message.event.model.ts
var TITLE_MAX2 = 120;
function assistantResponseEvent(target, input) {
  const metadata = {
  ...
  return {
    ...
    ...turnOf(target),
    lane: LANE.assistant,
    title: input.message ? ellipsize(input.message, TITLE_MAX2) : `Response (${input.stopReason})`,
    ...input.message ? { body: input.message } : {},
    metadata
  };
}
function assistantCommentaryEvent(target, input) {
  const metadata = {
    ...
    ...input.parentUuid !== void 0 ? { parentUuid: input.parentUuid } : {},
  ...
  return {
    ...
    ...turnOf(target),
    lane: LANE.assistant,
    title: ellipsize(input.text, TITLE_MAX2),
    body: input.text,
    metadata
  };
}

// src/domain/ingest/model/thought.event.model.ts
var TITLE_MAX3 = 120;
var THOUGHT_HEAD = 4096;
var THOUGHT_TAIL = 2048;
function thoughtLoggedEvent(target, input) {
  ...
  return {
    ...
    ...turnOf(target),
    lane: LANE.assistant,
    title: ellipsize(input.text, TITLE_MAX3),
    body: captured.body,
    metadata
  };
}

// src/domain/ingest/model/token.usage.event.model.ts
function tokenUsageEvent(target, input) {
  return {
    id: input.eventId,
    ...
    payload: {
      ...
      ...input.requestId !== void 0 ? { promptId: input.requestId } : {},
      metadata: {
...
// src/agent/claude-code/transcript/transcript.event.ts
var SOURCE = "claude-transcript";
function transcriptCommentaryId(sourceSessionId, entryUuid, contentIndex) {
...
function transcriptThoughtId(sourceSessionId, entryUuid, contentIndex) {
  return deterministicUlid([
    SOURCE,
    sourceSessionId,
    entryUuid,
...
function transcriptTokenUsageId(sourceSessionId, messageId) {
  return deterministicUlid([SOURCE, sourceSessionId, messageId, KIND.tokenUsage]);
}
function toTranscriptEvents(entries, sourceSessionId, target) {
  const commentary = /* @__PURE__ */ new Map();
  const thoughts = /* @__PURE__ */ new Map();
  const usageEvents = /* @__PURE__ */ new Map();
  for (const entry of entries) {
    ...
    const parentUuid = typeof entry.parentUuid === "string" ? entry.parentUuid : void 0;
    const requestId = typeof entry.requestId === "string" ? entry.requestId : void 0;
...
async function captureTranscriptCommentary(context, target, postEvents, cursorDir = resolveTranscriptCursorDir()) {
  const transcriptPath = context.agentTranscriptPath ?? context.transcriptPath;
  if (transcriptPath === void 0) return;
  const sourceSessionId = context.agentId !== void 0 ? subagentSessionId(context.agentId) : context.sessionId;
  try {
...
function onLifecycleEvent(hook, events) {
...
// src/agent/claude-code/hooks/Stop.ts
await runHook("Stop", {
  parse: readStop,
  handler: async (payload) => {
    ...
    const isSubagent = payload.agentId !== void 0;
    ...
    const runtimeSessionId = payload.agentId !== void 0 ? subagentSessionId(payload.agentId) : payload.sessionId;
    ...
    await onSessionEnd(claudeRuntime.session, {
      ...
      runtimeSource: claudeRuntime.runtimeSource,
      runtimeSessionId,
      summary: isSubagent ? `Subagent turn completed (${stopReason})` : `Assistant turn completed (${stopReason})`,
