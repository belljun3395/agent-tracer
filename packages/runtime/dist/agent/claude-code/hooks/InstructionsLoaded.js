...
function readStringArray(raw, field) {
  const value = raw[field];
  if (!Array.isArray(value)) return [];
  return value.flatMap((entry) => {
    const trimmed2 = toTrimmedString(entry);
...
function readInstructionsLoaded(raw) {
...
function instructionsLoadedEvent(target, input) {
  const relPath = relativeProjectPath(input.projectDir, input.filePath);
  const fileName = path11.basename(input.filePath);
  const metadata = {
  ...
  return {
    ...
    ...turnOf(target),
    lane: LANE.planning,
    title: input.loadReason === "compact" ? `Instructions reloaded: ${fileName}` : `Instructions loaded: ${fileName}`,
    body: relPath,
    metadata
  };
}

// src/agent/claude-code/hooks/InstructionsLoaded.ts
