...
var TODO_TOOLS = ["TodoWrite", "TaskCreate", "TaskUpdate"];
var STATUS_MAP = {
  pending: "added",
...
function readChildSessionId(toolResponse) {
...
function onToolUse(hook, call, target) {
  return hook.recordToolUse.execute(call, target);
}
function onTodoTool(hook, call, target, runtimeSessionId) {
  return hook.recordTodo.execute(call, target, runtimeSessionId);
}

// src/agent/claude-code/hooks/PostToolUse.ts
var AGENT_TOOL_NAME = "Agent";
var TODO_TOOL_NAMES = new Set(TODO_TOOLS);
