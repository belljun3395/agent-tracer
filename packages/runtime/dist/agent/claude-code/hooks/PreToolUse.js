...
function emitHints(hookEventName, hints) {
...
function onToolStart(hook, sessionId, toolUseId) {
...
// src/agent/claude-code/hooks/PreToolUse.ts
var SHELL_TOOLS = /* @__PURE__ */ new Set([TERMINAL_COMMAND_TOOL_NAME, POWERSHELL_TOOL_NAME]);
await runHook("PreToolUse", {
  ...
  handler: async (payload) => {
    ...
    if (!toolName) return;
    const command = SHELL_TOOLS.has(toolName) ? toTrimmedString(payload.toolInput["command"]) || void 0 : void 0;
    const questions = toolName === ASK_USER_QUESTION_TOOL_NAME ? readQuestions(payload.toolInput) : [];
    if (command === void 0 && questions.length === 0) return;
...
function readQuestions(toolInput) {
  ...
  return questions.flatMap((item) => {
    ...
    const text = toTrimmedString(item["question"]);
    return text ? [text] : [];
  });
}
