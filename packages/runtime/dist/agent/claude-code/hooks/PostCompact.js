...
function compactFinishedEvent(target, trigger, summary) {
  return contextEvent(
    target,
    "Context compacted",
...
// src/agent/claude-code/hooks/PostCompact.ts
var SUMMARY_MAX2 = 1e3;
await runHook("PostCompact", {
  parse: readPostCompact,
  handler: async (payload) => {
    const target = await ensureClaudeSession(payload.sessionId, void 0, {
      transcriptPath: payload.transcriptPath
    });
    const summary = toTrimmedString(payload.payload["compact_summary"], SUMMARY_MAX2);
    await onLifecycleEvent(claudeRuntime.ingest, [
      compactFinishedEvent(target, payload.trigger, summary || void 0)
    ]);
  }
});
