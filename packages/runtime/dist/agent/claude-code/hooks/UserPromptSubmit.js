...
function emitAgentContext(hookEventName, input) {
  const sections = [
...
// src/agent/claude-code/payload/session.payload.ts
...
var TITLE_MAX2 = 120;
function userMessageEvent(target, input) {
  const metadata = {
...
// src/domain/recipe/model/recipe.outcome.nudge.model.ts
function formatRecipeOutcomeNudge(recipeId) {
  return [
...
function onPendingRecipeMarkRequested(hook, taskId) {
...
await runHook("UserPromptSubmit", {
  ...
  handler: async (payload) => {
    ...
    const target = await ensureClaudeSession(
      payload.sessionId,
      systemNotification ? void 0 : ellipsize(payload.prompt, TITLE_MAX2),
      { transcriptPath: payload.transcriptPath }
    );
    ...
    const pendingMark = systemNotification ? void 0 : onPendingRecipeMarkRequested(claudeRuntime.recipeOutcomeMark, target.taskId);
    const recipeNudge = systemNotification ? "" : pendingMark !== void 0 ? formatRecipeOutcomeNudge(pendingMark.recipeId) : formatRecipeNudge();
