// src/claude-code/hooks/InstructionsLoaded.ts
...
await runHook("InstructionsLoaded", {
  ...
  handler: async (payload) => {
    ...
    const fileName = path4.basename(payload.filePath);
    const loadReason = payload.loadReason ?? "session_start";
    const memoryType = payload.memoryType ?? "Project";
    const title = loadReason === "compact" ? `Instructions reloaded: ${fileName}` : `Instructions loaded: ${fileName}`;
    const metadata = {
