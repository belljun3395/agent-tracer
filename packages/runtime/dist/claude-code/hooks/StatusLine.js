...
// src/claude-code/hooks/StatusLine.ts
...
async function main() {
  ...
  try {
    const result = await ensureRuntimeSession(sessionId, void 0, { resume: false });
  ...
  const usedPct = ctx?.used_percentage ?? void 0;
  const title = usedPct != null ? `Context ${Math.round(usedPct)}% used` : "Context snapshot";
  const metadata = {
