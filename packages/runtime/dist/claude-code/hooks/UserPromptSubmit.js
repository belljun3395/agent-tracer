...
// src/claude-code/hooks/lib/hook/hook.log.ts
var hookLog = claudeHookRuntime.logger.log;
var hookLogPayload = claudeHookRuntime.logger.logPayload;

// src/claude-code/hooks/lib/preprocessing-hints.ts
async function fetchPreprocessingHints(taskId, request) {
  if (!taskId) return [];
  try {
    ...
    for (const candidate of hints) {
      ...
      const h = candidate;
      if (typeof h["type"] !== "string" || typeof h["severity"] !== "string" || typeof h["title"] !== "string" || typeof h["message"] !== "string") continue;
...
function emitPreprocessingHints(hookEventName, hints) {
...
// src/claude-code/hooks/lib/recipe-context.ts
async function fetchRecipeMatches(prompt, options = {}) {
  ...
  try {
    ...
    for (const candidate of matches) {
      if (!candidate || typeof candidate !== "object") continue;
      const m = candidate;
      if (typeof m["recipeId"] !== "string" || typeof m["title"] !== "string" || typeof m["intent"] !== "string" || typeof m["description"] !== "string" || typeof m["summaryMd"] !== "string" || typeof m["score"] !== "number") continue;
      validated.push(candidate);
    }
    return validated;
  } catch (err) {
...
function emitRecipeContext(hookEventName, matches) {
...
function formatRecipeContext(matches) {
  const lines = ["<agent-tracer-recipes>"];
  lines.push(
  ...
  for (const m of matches) {
    ...
    if (summary) {
      const compact = summary.length > 600 ? summary.slice(0, 600) + "\u2026" : summary;
...
function readUserPromptSubmit(raw) {
...
// src/claude-code/hooks/UserPromptSubmit.ts
await runHook("UserPromptSubmit", {
  ...
  handler: async (payload) => {
    ...
    const phase = ids.taskCreated ? "initial" : "follow_up";
    const metadata = {
