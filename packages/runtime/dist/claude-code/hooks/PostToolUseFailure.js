...
// src/claude-code/hooks/util/payload.ts
...
function relativeProjectPath(filePath) {
  ...
  const relative2 = path.relative(PROJECT_DIR, filePath);
  if (!relative2) return "";
  const normalizedRelative = relative2.split(path.sep).join("/");
  if (normalizedRelative === ".." || normalizedRelative.startsWith("../") || path.isAbsolute(relative2)) {
    return filePath;
  }
...
function resolveDaemonHomeLayout(env = process.env) {
  ...
  const homeDir = path2.join(home, DAEMON_HOME_DIRNAME);
  const socketPath = explicitSocket ? explicitSocket : path2.join(homeDir, DAEMON_SOCKET_FILENAME);
  const logPath = path2.join(homeDir, DAEMON_LOG_FILENAME);
...
function daemonEntryPath() {
  return path2.join(runtimeRoot(), "src/shared/hook-runtime/local-daemon-entry.ts");
}
function tsxBinPath() {
  let current = runtimeRoot();
  for (; ; ) {
    const candidate = path2.join(current, "node_modules/.bin/tsx");
...
function maybeStartDaemon(layout) {
  ...
  if (isBun) {
    ...
    if (!tsx) return;
    executable = tsx;
    args = ["--tsconfig", path2.join(runtimeRoot(), "tsconfig.json"), daemonEntryPath()];
  }
...
var claudeHookRuntime = createHookRuntime({
  logFile: path3.join(PROJECT_DIR, ".claude", "hooks.log")
});
...
// src/shared/semantics/command-analysis.ts
...
function analyzeSed(base, args) {
  ...
  const targets = pathTargets(args.filter((arg) => !arg.startsWith("-") && extractSedLineRange(arg) === void 0));
  return withStep(base, {
...
function analyzeRipgrep(base, args) {
  ...
  const positional = stripOptionArguments(args, /* @__PURE__ */ new Set(["-g", "--glob", "--type", "-t", "--type-not", "-T", "-e", "--regexp"])).filter((arg) => !arg.startsWith("-"));
  if (filesMode) {
    return withStep(base, {
      operation: "list",
      effect: "read_only",
...
// src/shared/semantics/inference.command.ts
...
function explorationResult(commandEntity, analysis) {
  return {
    lane: "exploration",
    metadata: {
...
function executionResult(commandEntity, analysis, subtypeKey) {
  return {
    lane: "implementation",
    metadata: {
      subtypeKey,
...
function ruleCheckResult(commandEntity, analysis) {
  return {
    lane: "rule",
    metadata: {
...
function flattenOperations(analysis) {
...
function isLegacyExplorationProbe(normalized) {
...
// src/shared/semantics/inference.coordination.ts
function inferMcpSemantic(mcpServer, mcpTool, sourceToolName) {
  return {
    subtypeKey: "mcp_call",
...
function inferSkillSemantic(skillName, sourceToolName = "Skill") {
  return {
...
function inferAgentSemantic(entityName, sourceToolName = "Agent") {
  return {
...
function baseFileSemantic(subtypeKey, subtypeLabel2, operation, sourceTool, entityName) {
  return {
    subtypeKey,
...
// src/claude-code/hooks/PostToolUseFailure.ts
await runHook("PostToolUseFailure", {
  ...
  handler: async (payload) => {
    ...
    const description = payload.toolName === "Bash" ? toTrimmedString(payload.toolInput["description"]) : void 0;
    const command = payload.toolName === "Bash" ? toTrimmedString(payload.toolInput["command"]) : void 0;
    const filePath = payload.toolName === "Edit" || payload.toolName === "Write" ? toTrimmedString(payload.toolInput["file_path"]) || toTrimmedString(payload.toolInput["path"]) || "" : "";
    ...
    const classification = (() => {
      ...
      if (payload.toolName === "Edit" || payload.toolName === "Write") {
        return {
          lane: "implementation",
          metadata: inferFileToolSemantic(payload.toolName, relPath || void 0)
        };
      }
      if (payload.toolName === "Skill") {
        return {
          lane: "coordination",
          metadata: inferSkillSemantic(toTrimmedString(payload.toolInput["skill"]) || void 0, "Skill")
        };
      }
      if (payload.toolName === "Agent") {
        return {
          lane: "coordination",
          metadata: inferAgentSemantic(toTrimmedString(payload.toolInput["subagent_type"]) || void 0, "Agent")
        };
      }
      return null;
    })();
