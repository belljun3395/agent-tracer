...
function createStableTodoId(content, priority) {
...
function writeJsonFile(filePath, value, spacing) {
  ...
  const tempFilePath = path4.join(
    directory,
    `.${path4.basename(filePath)}.${process.pid}.${crypto3.randomUUID()}.tmp`
  );
  try {
    fs3.mkdirSync(directory, { recursive: true });
    fs3.writeFileSync(tempFilePath, JSON.stringify(value, null, spacing));
    fs3.renameSync(tempFilePath, filePath);
  } catch {
    try {
...
function isTodoStateSnapshot(value) {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return false;
  const v = value;
  if (!Array.isArray(v["todos"])) return false;
  return v["todos"].every((item) => {
...
function statePath(sessionId) {
  return path5.join(TODO_STATE_DIR, `${sessionId}.json`);
}
function loadTodoState(sessionId) {
  return readJsonFile(statePath(sessionId), isTodoStateSnapshot);
}
function saveTodoState(sessionId, snapshot) {
  writeJsonFile(statePath(sessionId), snapshot);
}

// src/claude-code/hooks/PostToolUse/_todo.ops.ts
var STATUS_MAP = {
  pending: "added",
...
function reconcileTodoWrite(toolInput, sessionId) {
  const rawTodos = Array.isArray(toolInput["todos"]) ? toolInput["todos"] : [];
  const currentItems = rawTodos.flatMap((todo) => {
  ...
  const previousByTodoId = new Map(
...
function extractTodoEvents(toolName, toolInput, sessionId) {
...
async function postTodoEvents({ payload, ids }) {
  ...
  await postTaggedEvents2(events.map((event) => {
    const metadata = {
      ...evidence,
      todoId: event.todoId,
...
// src/claude-code/hooks/PostToolUse/TaskCreate.ts
