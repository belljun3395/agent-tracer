...
// src/claude-code/hooks/PostToolUse/_explore.ops.ts
...
function sanitizeToolInputValue(value, maxValueLength, depth) {
...
function stringifyToolInput(input, maxValueLength = 1e4) {
  return Object.fromEntries(
...
// src/shared/semantics/inference.explore.ts
...
function toOptionalStringArray(value) {
  ...
  const items = value.map((v) => toTrimmedString(v)).filter((s) => Boolean(s));
...
async function postExploreToolEvent({ payload, ids }) {
  ...
  if (toolName === "Read") {
    ...
    title = `Read: ${path4.basename(relPath)}${rangeSuffix}`;
    body = `Reading ${relPath}${rangeSuffix}`;
    filePaths = filePath ? [filePath] : [];
    if (offset !== void 0) extras.readOffset = offset;
    if (limit !== void 0) extras.readLimit = limit;
...
Prompt: ${webPrompt}` : ""}`;
  ...
  const webQuery = isWebTool ? (toTrimmedString(payload.toolInput["query"]) || toTrimmedString(payload.toolInput["url"])).slice(0, MAX_PATH_LENGTH) : "";
  const entityName = filePaths[0];
  const queryOrUrl = isWebTool ? toTrimmedString(payload.toolInput["query"]) || toTrimmedString(payload.toolInput["url"]) : void 0;
...
async function postQuestionToolEvent({ payload, ids }) {
  const question = toTrimmedString(payload.toolInput["question"]);
  const options = Array.isArray(payload.toolInput["options"]) ? payload.toolInput["options"].filter((option) => typeof option === "string" && option.trim().length > 0) : [];
...
async function postPlanToolEvent({ payload, ids }) {
...
// src/claude-code/hooks/PostToolUse/AskUserQuestion.ts
