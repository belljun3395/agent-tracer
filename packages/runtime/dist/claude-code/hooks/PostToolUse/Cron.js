...
// src/claude-code/hooks/PostToolUse/Cron.ts
await runPostToolUseHook("Cron", async ({ payload, ids }) => {
  ...
  const title = toolName === "CronCreate" ? `Cron schedule: ${schedule || "?"}` : toolName === "CronDelete" ? `Cron delete: ${cronId || "?"}` : "Cron list";
  const body = toolName === "CronCreate" && promptArg ? `Schedule: ${schedule}
Prompt: ${promptArg}` : toolName === "CronDelete" ? `Cancel scheduled task ${cronId}` : "Enumerated scheduled tasks";
  const metadata = {
