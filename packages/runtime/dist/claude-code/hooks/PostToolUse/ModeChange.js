...
// src/claude-code/hooks/PostToolUse/ModeChange.ts
await runPostToolUseHook("ModeChange", async ({ payload, ids }) => {
  const toolName = payload.toolName;
  const path4 = toTrimmedString(payload.toolInput["path"]);
  let title;
  let body;
  let lane = LANE.planning;
  if (toolName === "EnterPlanMode") {
    title = "Enter plan mode";
    body = "Switched to plan mode";
  } else if (toolName === "EnterWorktree") {
    title = path4 ? `Enter worktree: ${path4}` : "Enter worktree";
    body = path4 ? `Switched into worktree at ${path4}` : "Switched into worktree";
