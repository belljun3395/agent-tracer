...
import { SessionLifecyclePolicy } from "../../src/application/services/session-lifecycle-policy.js";

describe("SessionLifecyclePolicy", () => {
  it("background 자식이 남아 있으면 primary assistant turn을 자동 완료하지 않는다", () => {
    expect(SessionLifecyclePolicy.shouldAutoCompletePrimary({
      taskKind: "primary",
      completeTask: true,
  ...
  it("idle 종료는 primary 작업을 waiting으로 보낸다", () => {
    expect(SessionLifecyclePolicy.shouldMovePrimaryToWaiting({
      taskKind: "primary",
      completeTask: false,
      runningSessionCount: 0,
