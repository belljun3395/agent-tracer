...
describe("setup:external Claude integration", () => {
  ...
  it("writes plugin-mode settings without legacy hooks or vendored scripts", async () => {
    ...
    // Generated settings should expose permissions only — hook registration is
    // delegated to the Claude Code plugin in .claude/plugin/.
    const generatedSettings = JSON.parse(
      await readFile(path.join(targetDir, ".claude", "settings.json"), "utf8")
    ) as { hooks?: unknown; permissions?: { defaultMode?: string; allow?: string[] } };

    expect(generatedSettings.hooks).toBeUndefined();
    expect(generatedSettings.permissions?.defaultMode).toBe("acceptEdits");
    expect(generatedSettings.permissions?.allow).toEqual(
      expect.arrayContaining(["WebSearch", "WebFetch"])
    );

    // No legacy vendored hook tree should be created.
    await expect(
      readFile(path.join(targetDir, ".agent-tracer", ".claude", "hooks", "common.ts"), "utf8")
    ).rejects.toThrow();
    await expect(
      readFile(path.join(targetDir, ".agent-tracer", ".claude", "hooks", "session_start.ts"), "utf8")
    ).rejects.toThrow();

    // The script should print the plugin install hint with an absolute path
    // pointing at .claude/plugin under the source repo.
  ...
  it("strips a legacy hooks block from a pre-existing settings.json", async () => {
    ...
    expect(generatedSettings.hooks).toBeUndefined();
    // User-provided permissions overrides should take precedence over defaults.
    expect(generatedSettings.permissions?.defaultMode).toBe("ask");
    expect(generatedSettings.permissions?.allow).toEqual(
      expect.arrayContaining(["WebSearch"])
    );
  }, 60_000);
});
