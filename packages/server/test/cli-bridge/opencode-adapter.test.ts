...
describe("OpenCodeAdapter", () => {
  ...
  it("starts opencode process and extracts session id", async () => {
    ...
    const { OpenCodeAdapter } = await import("../../src/application/cli-bridge/opencode-adapter.js");
  ...
  it("passes explicit model selection to opencode run", async () => {
    const { spawn } = await import("node:child_process");
    const { OpenCodeAdapter } = await import("../../src/application/cli-bridge/opencode-adapter.js");
    ...
    expect(spawn).toHaveBeenCalledWith(
      "opencode",
      ["run", "hello", "--format", "json", "--dir", "/repo", "--pure", "--model", "openai/gpt-5.4"],
      expect.objectContaining({ cwd: "/repo" })
    );
  });
