...
import { NoopRuleGenerationDispatcher } from "@monitor/rules-api/adapter/generation/noop.rule.generation.dispatcher.js";
...
// RuleGeneration은 로컬 플러그인이 실행하므로 noop 디스패처를 사용한다.
@Global()
@Module({
    providers: [
        TemporalClientProvider,
        NoopRuleGenerationDispatcher,
        TemporalTitleSuggestionDispatcher,
        TemporalRecipeScanDispatcher,
        TemporalTaskCleanupDispatcher,
        { provide: RULE_GENERATION_DISPATCHER, useExisting: NoopRuleGenerationDispatcher },
