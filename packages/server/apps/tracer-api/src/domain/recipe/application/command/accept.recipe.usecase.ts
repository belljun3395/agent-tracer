...
export class AcceptRecipeUseCase {
    ...
    private async applyInTransaction(tx: RecipeTx, userId: string, id: string, now: Date): Promise<RecipeEntity> {
        ...
        const parentCandidate = recipe.parentRecipeId !== null ? await tx.recipes.findById(recipe.parentRecipeId) : null;
        ...
        if (parent !== null) await tx.recipes.upsert(parent);
        await tx.recipes.upsert(recipe);
        await tx.searchOutbox.enqueue(enqueueRecipeIndex(userId, recipe.id, now));
        if (parent !== null) await tx.searchOutbox.enqueue(enqueueRecipeIndex(userId, parent.id, now));
        return recipe;
    }
}
