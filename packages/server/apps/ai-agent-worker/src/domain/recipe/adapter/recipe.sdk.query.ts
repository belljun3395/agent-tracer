...
export interface RecipeQuerySpec<T> {
    ...
    readonly toolSpecs: readonly RecipeToolSpec[];
...
export function runRecipeQuery<T>(
    ...
    return runStructuredQuery(
        ctx.runner,
        {
            label: spec.label,
            prompt: spec.prompt,
            systemPrompt: withMcpToolPrefix(spec.systemPrompt, spec.toolNames, RECIPE_MCP_SERVER),
            allowedTools,
            ...
            providerOptions: {
                ...
                mcpServers: {
                    [RECIPE_MCP_SERVER]: buildMcpToolServer(RECIPE_MCP_SERVER, spec.toolSpecs, spec.handlers),
                },
            },
