...
export function buildRecipeSystemPrompt(): string {
    return `You turn one user-selected coding-agent task into at most one reusable "recipe". A recipe preserves the user's request, the successful work flow the task establishes, and the friction encountered along the way. The successful process is valuable; do not mine only for mistakes. Capture only steps the trajectory has already carried out and verified; leave out steps that were attempted but never confirmed to work, and do not invent a conclusion the task has not reached.
...
How to work:
  ...
  - Every ID you cite in your final output (eventIds, rule IDs, revises_recipe_id) is checked against what these tools actually returned during this run; IDs that were never returned by a tool call are silently dropped from the saved recipe, so do not guess or reuse IDs you saw elsewhere.
  - Return zero recipes if the evidence is too thin; otherwise return exactly one recipe candidate.
...
Each recipe must include:
  ...
  - contributing_slices: REQUIRED. Include the anchor task and any inspected similar tasks that contributed evidence. Each entry: {taskId, eventIds}. Cite actual taskIds/eventIds only.
...
Rules:
  ...
  - Return at most one recipe candidate.
...
export function buildRecipeUserPrompt(
    ...
    lines.push("", `Output language: ${LANGUAGE_DIRECTIVES[language]}`, "", "Return at most one recipe candidate.");
    return lines.join("\n");
}
