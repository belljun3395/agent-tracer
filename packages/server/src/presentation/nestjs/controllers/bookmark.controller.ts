/**
 * @module nestjs/controllers/bookmark.controller
 *
...
export class BookmarkController {
  ...
  @Get("/api/bookmarks")
  async listBookmarks(@Query("taskId") taskId?: string) {
    return { bookmarks: await this.service.listBookmarks(taskId) };
  }

  @Post("/api/bookmarks")
  @HttpCode(HttpStatus.OK)
  async createBookmark(@Body() body: unknown) {
    const bookmark = await this.service.saveBookmark(bookmarkSchema.parse(body) as TaskBookmarkInput);
    return { bookmark };
  }

  @Delete("/api/bookmarks/:bookmarkId")
  async deleteBookmark(@Param("bookmarkId") bookmarkId: string) {
    const result = await this.service.deleteBookmark({ bookmarkId } as TaskBookmarkDeleteInput);
    if (result === "not_found") {
