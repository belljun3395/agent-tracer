 ...
 *
 * Facade service for monitoring business logic.
 * Delegates to specialized services: TaskLifecycleService, EventLoggingService, WorkflowEvaluationService.
 *
...
export class MonitorService {
  ...
  constructor(private readonly ports: MonitorPorts) {
    ...
    this.eventLogging = new EventLoggingService(
      ports,
      this.taskLifecycle
    );
    this.workflowEvaluation = new WorkflowEvaluationService(
      ports,
      this.taskLifecycle
    );
  }
  ...
  async logAssistantResponse(
    input: TaskAssistantResponseInput
  ): Promise<RecordedEventEnvelope> {
    const task = await this.taskLifecycle.requireTask(input.taskId);
  ...
  async endSession(
    input: TaskSessionEndInput
  ): Promise<{ sessionId: string; task: MonitoringTask }> {
  ...
  async logTerminalCommand(
    input: TaskTerminalCommandInput
  ): Promise<RecordedEventEnvelope> {
    const task = await this.taskLifecycle.requireTask(input.taskId);
  ...
  async logRule(
    ...
    const updatedTask = await this.applyRuleEnforcementStatus(
      input,
      result.events.length > 0 ? { ...task, ...result } : task
    );
    return {
      ...result,
      task: updatedTask,
  ...
  private resolveRuleGuardStatus(
    input: TaskRuleInput
  ): MonitoringTask["status"] | undefined {
  ...
  async logAsyncLifecycle(
    input: TaskAsyncLifecycleInput
  ): Promise<RecordedEventEnvelope> {
    const task = await this.taskLifecycle.requireTask(input.taskId);
  ...
  async getTaskLatestRuntimeSession(
    taskId: string
  ): Promise<{ runtimeSource: string; runtimeSessionId: string } | null> {
  ...
  async saveBookmark(input: TaskBookmarkInput): Promise<BookmarkRecord> {
    ...
    if (event && event.taskId !== task.id) {
      throw new Error(
        `Event ${event.id} does not belong to task ${task.id}`
      );
    }
    ...
    const bookmark = await this.ports.bookmarks.save({
      ...
      title:
        input.title?.trim() || event?.title || task.title,
  ...
  async updateEvent(
    ...
    const currentDisplayTitle =
      typeof event.metadata["displayTitle"] === "string"
    ...
    const updated = await this.ports.events.updateMetadata(
      event.id,
      nextMetadata
    );
    if (updated) {
  ...
  async searchWorkflowLibrary(
    ...
    return this.workflowEvaluation.searchWorkflowLibrary(
      query,
      rating,
      limit
    );
  ...
  async searchSimilarWorkflows(
    ...
    return this.workflowEvaluation.searchSimilarWorkflows(
      query,
      tags,
      limit
    );
