/**
 * @module application/services/event-logging-service
 *
 * Event logging: terminal commands, tools, exploration, plans, actions, verification,
 * rules, async lifecycle, questions, todos, thoughts, agent activity, user messages, assistant responses.
 */
...
export class EventLoggingService {
  ...
  async logUserMessage(
    ...
    if (
      input.captureMode === "derived" &&
      !input.sourceEventId
    ) {
      throw new Error(
        "sourceEventId is required when captureMode is 'derived'."
      );
    }
    const phase =
      input.phase ??
    ...
    // EventRecorder saves input.filePaths to metadata.filePaths, so pass at top-level.
    const existingFilePaths = Array.isArray(input.metadata?.["filePaths"])
      ? (input.metadata?.["filePaths"] as string[]).filter(
          (p): p is string => typeof p === "string"
        )
      : undefined;
    const filePaths =
      existingFilePaths && existingFilePaths.length > 0
        ? existingFilePaths
        : input.body
  ...
  async logToolUsed(
    ...
    return this.withSession(input, (sid) => ({
      ...
      metadata: TMF.build(
        { ...(input.metadata ?? {}), toolName: input.toolName },
        input
      ),
  ...
  async logPlan(
    ...
    return this.withSession(input, (sid) => ({
      ...
      metadata: TMF.build(
        { ...(input.metadata ?? {}), action: input.action },
        input
      ),
      ...
      metadata: TMF.build(
        {
          ...
          action: input.action,
          result: input.result,
          verificationStatus: TMF.normalizeVerificationStatus(
            input.status ?? input.result
          ),
        },
        input
      ),
  ...
  async logAgentActivity(
    ...
    return this.withSession(input, (sid) => ({
      ...
      lane:
        (input.lane as TimelineLane | undefined) ?? "coordination",
      title:
        input.title ??
  ...
  private async withSession(
    ...
    buildEvent: (sessionId: string | undefined) => GenericEventInput
  ): Promise<RecordedEventEnvelope> {
    const sessionId = await this.resolveSessionId(
      input.taskId,
      input.sessionId
  ...
  private async recordWithDerivedFiles(
    input: GenericEventInput
  ): Promise<RecordedEventEnvelope> {
    return await this.recorder.recordWithDerivedFiles(input);
  }
}
