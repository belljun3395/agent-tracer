/**
 * @module application/services/task-lifecycle-service
 *
 * Task lifecycle management: start, complete, error, link, delete, rename, patch.
 * Handles task status transitions and session management.
 */
...
export class TaskLifecycleService {
  ...
  async endSession(
    ...
    const runningCount = await this.ports.sessions.countRunningByTaskId(
      input.taskId
    );
    if (!input.sessionId && runningCount > 1) {
      throw new Error(
        `sessionId is required to end one of multiple running sessions for task: ${input.taskId}`
      );
    }
    ...
    await this.ports.sessions.updateStatus(
      sessionId,
      "completed",
      endedAt,
      input.summary
    ...
    const postRunning = await this.ports.sessions.countRunningByTaskId(
      task.id
    );
    const taskKind = task.taskKind ?? "primary";
    const hasRunningBackgroundDescendants =
      taskKind === "primary"
  ...
  async ensureRuntimeSession(
    ...
    const existingTaskId =
      await this.ports.runtimeBindings.findTaskId(
        input.runtimeSource,
        input.runtimeSessionId
      );
    if (existingTaskId) {
  ...
  async endRuntimeSession(
    ...
    if (!binding) {
      ...
      if (input.completeTask === true) {
        ...
        if (taskId) {
          ...
          await this.ports.runtimeBindings.delete(
            input.runtimeSource,
            input.runtimeSessionId
          );
        }
    ...
    const session = await this.ports.sessions.findById(
      binding.monitorSessionId
    );
    ...
    await this.ports.runtimeBindings.clearSession(
      input.runtimeSource,
      input.runtimeSessionId
    );
    await this.completeBgTasks(input.backgroundCompletions);
    ...
    if (input.completeTask === true && task) {
      if (
        shouldAutoCompletePrimary({
          taskKind: task.taskKind ?? "primary",
          completeTask: true,
          runningSessionCount: await this.ports.sessions.countRunningByTaskId(
            binding.taskId
          ),
          completionReason: input.completionReason,
          hasRunningBackgroundDescendants,
  ...
  async renameTask(input: TaskRenameInput): Promise<MonitoringTask | null> {
    ...
    await this.ports.tasks.updateTitle(
      input.taskId,
      nextTitle,
      createTaskSlug({ title: nextTitle }),
  ...
  async updateTask(input: TaskPatchInput): Promise<MonitoringTask | null> {
    ...
    const hasNewTitle =
      titleUpdate !== undefined && titleUpdate !== task.title;
    const hasNewStatus =
    ...
    const updated = await this.ports.tasks.upsert({
      ...task,
      taskKind: task.taskKind ?? "primary",
      ...(hasNewTitle && titleUpdate !== undefined
        ? { title: titleUpdate, slug: createTaskSlug({ title: titleUpdate }) }
        : {}),
      ...(hasNewStatus && input.status !== undefined
        ? { status: input.status }
        : {}),
      updatedAt: new Date().toISOString(),
    });
  ...
  private async completeTaskIfIncomplete(
    input: TaskCompletionInput
  ): Promise<void> {
    const task = await this.ports.tasks.findById(input.taskId);
    if (
  ...
  private async hasRunningBackgroundDescendants(
    taskId: string
  ): Promise<boolean> {
    const stack = [taskId];
  ...
  private async finishTask(
    ...
    const finalTask =
      (await this.ports.tasks.findById(input.taskId)) ?? task;
    this.ports.notifier.publish(
      status === "completed"
    ...
    const event = await this.recorder.record({
      ...
      kind,
      title:
        status === "completed" ? "Task completed" : "Task errored",
