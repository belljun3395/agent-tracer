...
export class SqliteBookmarkRepository implements IBookmarkRepository {
    ...
    `).run({
        ...
        const row = this.db
            .prepare<{
            id: string;
        }, BookmarkRow>(`${BOOKMARK_SELECT} where b.id = @id`)
            .get({ id: input.id });
        if (row) {
    ...
    async findByTaskId(taskId: string): Promise<readonly BookmarkRecord[]> {
        return this.db
            .prepare<{
            taskId: string;
        }, BookmarkRow>(`${BOOKMARK_SELECT} where b.task_id = @taskId order by datetime(b.updated_at) desc`)
            .all({ taskId })
            .map(mapBookmarkRow);
    }
    async findAll(): Promise<readonly BookmarkRecord[]> {
        return this.db
            .prepare<[
        ], BookmarkRow>(`${BOOKMARK_SELECT} order by datetime(b.updated_at) desc`)
            .all()
            .map(mapBookmarkRow);
    }
