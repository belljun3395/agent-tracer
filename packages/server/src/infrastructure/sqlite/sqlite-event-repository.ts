...
export class SqliteEventRepository implements IEventRepository {
    ...
    async findById(id: MonitorEventId): Promise<TimelineEvent | null> {
        const row = this.db
            .prepare<{
            id: string;
        }, EventRow>("select * from timeline_events where id = @id")
            .get({ id });
    ...
    async findByTaskId(taskId: MonitorTaskId): Promise<readonly TimelineEvent[]> {
        return this.db
            .prepare<{
            taskId: string;
        }, EventRow>("select * from timeline_events where task_id = @taskId order by datetime(created_at) asc")
            .all({ taskId })
            .map(mapEventRow);
    }
    ...
    private loadSearchDocuments(scope: SearchDocumentScope, taskId?: string | null): readonly SearchDocumentRow[] {
        return this.db
            .prepare<{
            scope: SearchDocumentScope;
            taskId: string | null;
...
function rankSearchDocuments(rows: readonly SearchDocumentRow[], normalizedQuery: string, queryVector: Float32Array | null, limit: number): readonly SearchDocumentRow[] {
...
function scoreLexicalMatches(rows: readonly SearchDocumentRow[], normalizedQuery: string): readonly {
...
function scoreSemanticMatches(rows: readonly SearchDocumentRow[], queryVector: Float32Array): readonly {
    row: SearchDocumentRow;
    score: number;
}[] {
    return rows
