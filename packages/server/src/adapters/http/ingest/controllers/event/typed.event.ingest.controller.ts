...
import { ClassifyTerminalLaneUseCase } from "~application/rules/index.js";
import {
...
@Controller("ingest/v1/events/typed")
export class TypedEventIngestController {
    ...
    // terminal.command events with lane classification
    @Post("tool/activity")
    ...
    // plan/action/verification/rule/thought/context events
    @Post("workflow")
    @HttpCode(HttpStatus.OK)
    ...
    // user.message / assistant.response / question / todo events
    @Post("conversation")
    @HttpCode(HttpStatus.OK)
    ...
    // session.ended / instructions.loaded events
    @Post("lifecycle")
    @HttpCode(HttpStatus.OK)
