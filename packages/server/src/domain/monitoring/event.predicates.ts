...
const CONVERSATION_SET      = new Set<string>(CONVERSATION_EVENT_KINDS);
const COORDINATION_ACT_SET  = new Set<string>(COORDINATION_EVENT_KINDS);
...
export function isConversationEvent(e: WithKind): boolean    { return CONVERSATION_SET.has(e.kind); }
export function isCoordinationActivityEvent(e: WithKind): boolean { return COORDINATION_ACT_SET.has(e.kind); }
export function isTelemetryEvent(e: WithKind): boolean       { return TELEMETRY_SET.has(e.kind); }
...
export function isRuleOrVerificationEvent(e: WithKind): boolean {
    return e.kind === KIND.ruleLogged || e.kind === KIND.verificationLogged;
}

// --- Individual kind type guards ---

export function isTokenUsageEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.tokenUsage } {
    return e.kind === KIND.tokenUsage;
}
...
export function isAssistantResponseEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.assistantResponse } {
...
export function isFileChangedEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.fileChanged } {
    return e.kind === KIND.fileChanged;
}
export function isInstructionsLoadedEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.instructionsLoaded } {
    return e.kind === KIND.instructionsLoaded;
}
export function isQuestionLoggedEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.questionLogged } {
    return e.kind === KIND.questionLogged;
}
export function isTodoLoggedEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.todoLogged } {
    return e.kind === KIND.todoLogged;
}
export function isThoughtLoggedEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.thoughtLogged } {
    return e.kind === KIND.thoughtLogged;
}
export function isContextSavedEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.contextSaved } {
    return e.kind === KIND.contextSaved;
}
export function isTerminalCommandEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.terminalCommand } {
    return e.kind === KIND.terminalCommand;
}
export function isVerificationLoggedEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.verificationLogged } {
    return e.kind === KIND.verificationLogged;
}
export function isToolUsedEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.toolUsed } {
    return e.kind === KIND.toolUsed;
}
export function isRuleLoggedEvent<T extends WithKind>(e: T): e is T & { kind: typeof KIND.ruleLogged } {
