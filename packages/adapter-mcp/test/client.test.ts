...
import { createMonitorMcpServer } from "../src";
describe("MonitorClient", () => {
    ...
    it("baseUrl 기본값이 적용된다", () => {
        ...
        if (saved !== undefined)
            process.env.MONITOR_BASE_URL = saved;
    });
...
describe("MCP tool registry — canonical tools", () => {
    it("monitor_user_message 가 /api/user-message 로 POST한다", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://localhost:3847/api/user-message", expect.objectContaining({ method: "POST" }));
    });
    it("monitor_session_end 가 /api/session-end 로 POST한다", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://localhost:3847/api/session-end", expect.objectContaining({ method: "POST" }));
    ...
    it("monitor_runtime_session_ensure 가 /api/runtime-session-ensure 로 POST한다", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://localhost:3847/api/runtime-session-ensure", expect.objectContaining({ method: "POST" }));
    });
    it("monitor_runtime_session_end 가 /api/runtime-session-end 로 POST한다", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://localhost:3847/api/runtime-session-end", expect.objectContaining({ method: "POST" }));
        const requestInit = fetchMock.mock.calls[0]?.[1] as {
            body?: string;
        };
        expect(requestInit.body).toBe(JSON.stringify({
    ...
    it("monitor_assistant_response 가 /api/assistant-response 로 POST한다", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://localhost:3847/api/assistant-response", expect.objectContaining({ method: "POST" }));
    });
    ...
    it("monitor_task_link 가 /api/task-link 로 POST한다", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://localhost:3847/api/task-link", expect.objectContaining({ method: "POST" }));
    });
});
describe("MCP tool registry — question/todo/thought tools", () => {
    it("monitor_question 이 /api/question 으로 POST한다", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://localhost:3847/api/question", expect.objectContaining({ method: "POST" }));
    });
    it("monitor_todo 이 /api/todo 로 POST한다", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://localhost:3847/api/todo", expect.objectContaining({ method: "POST" }));
    });
    it("monitor_thought 이 /api/thought 로 POST한다", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://localhost:3847/api/thought", expect.objectContaining({ method: "POST" }));
    });
    it("monitor_agent_activity 가 /api/agent-activity 로 POST한다", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://localhost:3847/api/agent-activity", expect.objectContaining({ method: "POST" }));
    });
});
