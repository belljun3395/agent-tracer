...
export function isInstructionsBurstEvent(event: TimelineEvent): boolean {
...
export function groupInstructionsBursts(
    ...
    while (i < sorted.length) {
        ...
        const burst: TimelineEvent[] = [event];
        let j = i + 1;
...
function isGroupableInstruction(event: TimelineEvent): boolean {
    return event.kind === "instructions.loaded" && !isInstructionsBurstEvent(event);
}

function buildBurstEvent(burst: readonly TimelineEvent[]): TimelineEvent {
    const first = burst[0];
