...
> **구조 변경 (2026-06)**: bounded context 들이 별도 npm 패키지로 분리됐다. 아래 문서의
> `packages/server/src/<context>/` · `~<module>/` 표기는 이제 다음 패키지로 매핑된다.
> 레이어링·contract·명명 규칙(본문)은 그대로 유효하다. 모듈 간 import 는
> `@monitor/<context>-api/<subpackage>/public/...` 형태를 쓴다.
>
> | 옛 표기 | 현재 패키지 |
> |---|---|
> | `~activity/{session,event}` | `@monitor/activity-api` (`packages/activity-api`) |
> | `~work/{task,turn}` | `@monitor/work-api` (`packages/work-api`) |
> | `~governance/{rule,verification,…}` | `@monitor/governance-api` (`packages/governance-api`) |
> | identity | `@monitor/identity-api` (`packages/identity-api`) |
> | 합성 루트 + HTTP edge (`main/`·`adapters/`·`config/`) | `@monitor/api-gateway` (`packages/api-gateway`) |
> | WS edge (broadcaster·redis fan-out·WS 서버) | `@monitor/ws-gateway` (`packages/ws-gateway`) |
> | platform 공용 (kernel·contracts·llm·mcp) | `@monitor/shared` (`packages/shared`) |

## 현재 적용 모듈 — 3개 bounded context, 10개 sub-package
...
| **governance** | rule + verification + settings + task-cleanup + rule-generation + recipe | `~governance/{rule,verification,settings,task-cleanup,rule-generation,recipe}/` | rule/verification = 핵심 (rule definition + turn evaluation/verdicts, 양방향 coupling은 intra-context). settings·task-cleanup·rule-generation·recipe = 이후 추가된 운영 sub-package, 동일 vertical-slice 패턴을 따른다. |

> **reference 구현**은 아래 [Reference Implementation](#reference-implementation) 표의 핵심 6개 (session/event/task/turn/rule/verification). governance 운영 sub-package 4개(settings/task-cleanup/rule-generation/recipe)도 같은 패턴을 따른다.
...
```
packages/
├── activity-api/        # @monitor/activity-api — SessionModule + EventModule
│   └── src/{session,event}/      # 각 vertical slice (domain/repository/...)
├── work-api/            # @monitor/work-api — TaskModule + TurnModule
│   └── src/{task,turn}/
├── governance-api/      # @monitor/governance-api — Rule/Verification + 운영 모듈
│   └── src/{rule,verification,settings,task-cleanup,rule-generation,recipe}/
├── identity-api/        # @monitor/identity-api — user/identity
├── shared/              # @monitor/shared — kernel/contracts/llm/mcp
├── ws-gateway/          # @monitor/ws-gateway — WS edge (broadcaster, redis fan-out)
└── api-gateway/         # @monitor/api-gateway — 합성 루트 + HTTP edge
    └── src/{main,adapters,config}/   # composition root, presentation, config
```
...
| **Event sourcing append-only log** | raw `db.prepare(...)` | `~activity/event/repository/event-store/event.store.ts` | 고성능 append + 단순 row 모델. ORM abstraction 가치 없음. |
| **FTS5 search** | raw `db.prepare(...)` + `MATCH` | `~activity/event/repository/search/` | TypeORM이 SQLite virtual table (FTS5) 잘 못 다룸. |
...
| 노출 종류 | 위치 | 예시 |
|---|---|---|
| 도메인 타입 (interface, string union) | `~<module>/public/types/<module>.types.ts` | `~activity/event/public/types/event.types.ts` (TimelineEvent, TimelineLane 등) |
| 도메인 상수 | `~<module>/public/types/<module>.const.ts` | `~activity/event/public/types/event.const.ts` (KIND, EVENT_LANES 등) |
| 도메인 predicate / pure helper | `~<module>/public/predicates.ts`, `~<module>/public/helpers.ts` | `~activity/event/public/predicates.ts` (isInternalEvent, isUserLane 등) |
| 에러 클래스 | `~<module>/public/errors.ts` | `~governance/rule/public/errors.ts` (InvalidRuleError, RuleNotFoundError) |

**원칙**: re-export 파일은 thin (구현 없음, `export ... from ...` 만). 모듈 internal에서 변경이 일어나도 public 표면이 안정되도록.

```ts
// ~activity/event/public/types/event.types.ts (thin barrel)
export type { TimelineEvent } from "~activity/event/domain/model/timeline.event.model.js";
export type { TimelineLane } from "~activity/event/domain/common/type/event.kind.type.js";
// ...
```
...
import { isInternalEvent } from "~activity/event/public/predicates.js";

// 다른 모듈 ❌
import type { TimelineEvent } from "~activity/event/domain/model/timeline.event.model.js";  // internal 침범
```
...
```ts
// ~governance/rule/public/errors.ts
export { RuleNotFoundError, InvalidRuleError } from "../common/errors.js";

// ~main/presentation/filters/zod-exception.filter.ts
import { RuleNotFoundError } from "~governance/rule/public/errors.js";  // ✅ public surface
// import from "~governance/rule/common/errors.js"  ❌ — internal 침범
```
...
3개 bounded context의 핵심 6개 sub-package가 이 패턴의 reference 구현. 각자 다른 측면을 보여줌 (governance 의 운영 sub-package 4개도 동일 패턴 적용):
