...
대화 내용의 기준 데이터는 공통 채팅 DB에 저장하며 체크포인트는 LangGraph 내부 실행 상태만 소유한다. 따라서 SDK에서 LangGraph로 바꾸면 LangGraph가 공통 DB의 SDK 답변까지 복원하고, LangGraph에서 SDK로 바꾸면 SDK가 공통 DB의 LangGraph 답변까지 모델 입력에 조립한다.
