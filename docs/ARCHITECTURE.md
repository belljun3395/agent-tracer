...
> Current migration phase: **Phase 7 (MCP driving adapter normalized: `adapter-mcp`)**
...
- Enforced as errors: `domain-is-pure`, `classification-depends-on-domain-only`, `application-inner-ring`, `no-cross-adapter`, `hook-plugin-wire-only` (Phase 6), `adapter-mcp-inner-ring` (Phase 7).
