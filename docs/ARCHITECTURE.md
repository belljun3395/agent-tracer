...
> Current migration phase: **Phase 8 (Web packages normalized: `web-app`, factory UI stores, `web-isolated` locked)**
...
- Enforced as errors: `domain-is-pure`, `classification-depends-on-domain-only`, `application-inner-ring`, `no-cross-adapter`, `hook-plugin-wire-only` (Phase 6), `adapter-mcp-inner-ring` (Phase 7), `web-isolated` (Phase 8c).
- Remaining rules (`application-no-adapter`, `no-subpath-imports`) stay in warning-mode until Phase 9 lock.
