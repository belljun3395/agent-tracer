...
그 밖의 경우에만 기존처럼 anchor부터 태스크의 현재 끝까지 전체 창을 읽어 판정을 전진시키고,
...
판정은 이벤트 원장에서 다시 만들 수 있는 조회 모델이고, high-water mark도 그 성질을 깨지 않는다.
재생은 이벤트 원장을 seq 순서로 흘리며 턴을 차례로 닫고, 그때마다 판정을 전진시킨다. high-water
mark는 매 전진에서 그 시점 창의 최대 seq로 결정되므로, 같은 이벤트 원장을 재생하면 같은 값이 채워진다.
