...
즉, 단일 event detail뿐 아니라 task-level summary, observability diagnostics,
workflow evaluation까지 이 패널이 함께 가진다.

### Exploration 탭 구조

`Exploration` 탭은 내부적으로 4개 섹션으로 구성된다.

| 섹션 | 컴포넌트 | 내용 |
|------|----------|------|
| Overview | `ExplorationInsightCard` | Total Explorations, Unique Files, Web Lookups 카운트 + Tool Breakdown |
| Web Lookups | `WebLookupsCard` | WebSearch/WebFetch 쿼리·URL 목록 (toolName, 횟수, 마지막 조회 시각) |
| Explored Files | `DetailExploredFiles` | 에이전트가 읽은 파일 경로 목록 (정렬·compact 관계 포함) |
| Mentioned Files | `MentionedFilesVerificationCard` | assistant 응답에 언급된 파일의 실존 여부 검증 |
...
`collectWebLookups(timeline)` 함수는 exploration 레인에서 `metadata.webUrls` 배열이 있는 이벤트를 추출해 URL별로 집계한다.

```ts
interface WebLookupStat {
  url: string;            // 검색 쿼리 또는 URL
  toolName: string;       // "WebSearch" | "WebFetch"
  count: number;          // 해당 URL이 조회된 횟수
  firstSeenAt: string;    // ISO timestamp
  lastSeenAt: string;     // ISO timestamp
}
```

`ExplorationInsight`에도 `uniqueWebLookups: number` 필드가 추가되어 Overview 카드에 표시된다.
...
### task observability read model이 붙었다

`EventInspector`는 이제 선택된 task에 대해 `/api/tasks/:taskId/observability`를 읽고,
phase breakdown, trace links, action-registry gaps, prompt/question/todo 신호를
별도 `Flow` / `Health` 탭으로 렌더링한다.
