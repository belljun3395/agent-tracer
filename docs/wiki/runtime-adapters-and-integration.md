...
| 방식 | 대표 런타임 | 특징 |
| --- | --- | --- |
| hook | Claude Code | runtime lifecycle을 자동으로 따라가며 raw prompt, tool use, subagent를 잘 관찰 |
| plugin | OpenCode | typed hook + event callback을 함께 써서 richer signal을 얻기 좋음 |
| skill + MCP | Codex | 자동 관측은 적지만 수동 기록 경로가 가장 명시적이고 통제 가능 |
...
Codex는 hook 수준 자동 관찰이 없기 때문에, skill이 언제 `monitor_user_message`,
`monitor_explore`, `monitor_assistant_response`를 호출할지 문서와 스킬 지침이 중요하다.
