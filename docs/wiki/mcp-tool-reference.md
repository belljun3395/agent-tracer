...
Lifecycle 7개 도구를 제외한 나머지 도구들은 모두 `POST /ingest/v1/events`
배치 엔드포인트로 전송한다. 각 도구는 `events: [{ kind: "...", ...input }]` 형태로
단일 이벤트를 감싸서 보낸다.
...
`monitor_session_end` 은 lifecycle 경로(`/api/session-end`)를 그대로 사용한다.

## 유지보수 메모

- 새 tool을 추가하면 `schemas.ingest.ts`의 kind 목록, `event-ingestion-service.ts`의 dispatch, guide 문서까지 같이 봐야 한다.
