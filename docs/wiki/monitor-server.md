...
### observability read model이 추가됐다

`MonitorService`는 이제 task-level `analyzeTaskObservability()`와
overview-level `analyzeObservabilityOverview()`를 통해 별도 진단 read model을 만든다.
`GET /api/tasks/:taskId/observability`, `GET /api/observability/overview`,
그리고 `GET /api/overview`의 `observability` snapshot이 이 경로를 사용한다.
