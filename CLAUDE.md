...
Claude Code, Codex 등 AI CLI 에이전트의 활동을 실시간으로 추적하는
...
| `@monitor/claude-plugin` | `.claude/plugin/` | Claude Code 플러그인 (hook 등록 + tsx 런너) |
...
## Claude Code 플러그인 (`.claude/plugin/`)

이 저장소는 Claude Code **plugin** 으로 Agent Tracer에 연결됩니다.
`.claude/settings.json`은 `permissions` 만 선언하고, hook 등록은 100% plugin이 담당합니다.

```bash
# 로컬 개발용 실행 (영구 설치는 marketplace 통해서)
claude --plugin-dir .claude/plugin
# 또는 alias 등록:
alias claude='claude --plugin-dir /path/to/agent-tracer/.claude/plugin'
```

플러그인 구조:

- `.claude-plugin/plugin.json`: 매니페스트 (`name`, `version`, `hooks` 경로)
- `hooks/hooks.json`: SessionStart, UserPromptSubmit, Pre/PostToolUse 등 16개 hook 등록
- `bin/run-hook.sh`: tsx 실행 wrapper (`${CLAUDE_PLUGIN_ROOT}/node_modules/tsx` 우선, `npx tsx` fallback)
...
세션 캐시와 디버그 로그는 plugin 디렉터리가 아니라 **유저 프로젝트의 `.claude/`** 에 남습니다
(`common.ts` 가 `CLAUDE_PROJECT_DIR` 환경변수를 사용).

**모니터 서버 포트**: `MONITOR_PORT` 환경변수 또는 기본값 `3847`

훅 디버그 로그: `${CLAUDE_PROJECT_DIR}/.claude/hooks.log` (NODE_ENV=development 시 활성화)
...
### 훅 수정 시
`.claude/plugin/hooks/*.ts` 파일은 `bin/run-hook.sh` 가 `tsx`로 직접 실행 (트랜스파일 없이).
`common.ts`의 API 호출 timeout은 1초 — 훅이 느리면 Claude 응답이 지연됨.
새 hook 이벤트를 추가하려면 `hooks/hooks.json` 에 한 줄 추가하면 됨.
...
```bash
npm run setup:external  # Claude/Codex 연동 파일 자동 생성
```
