...
Codex의 native hook 경로와 별개로, thread/topic 단위 semantic tracing을 담당하는 MCP 경로.
Codex native hooks도 `user.message`, Bash, transcript backfill, turn-local
`assistant.response`를 남길 수 있지만, 같은 task 재사용과 planning/context
추적은 이 skill 경로가 캐노니컬이다.
...
6. 최종 답변 직전에 `monitor_assistant_response`로 assistant 응답을 기록.
   - Codex skill 경로에서 final answer text를 남기는 canonical path다.
...
- `monitor_assistant_response` — 최종 user-facing 답변을 `assistant.response` 이벤트로 기록
  - native hook도 `Stop.last_assistant_message`로 turn-local `assistant.response`를 남길 수 있지만, skill 쪽이 thread/topic 기준 canonical 경로다
  - `source`는 보통 `codex-skill`
  - final answer text가 중요하면 이 경로를 생략하지 않는다
...
- `monitor_rule` — rule 관련 이벤트. `action` + `ruleId` + `severity` + `status` 필수
  - raw prompt gap은 `UserPromptSubmit`와 transcript backfill 모두 실패한 경우에만 명시한다
...
- **assistant response**: 답변 텍스트를 남기려면 `monitor_assistant_response`를 반드시 호출한다

## Hooks와의 관계

- Codex hook 경로(`codex-hook`)는 `user.message`, Bash, transcript backfill 같은 저수준 이벤트를 자동 관찰한다.
- `Stop.last_assistant_message`가 있으면 hook 경로도 turn-local `assistant.response`를 남긴다.
- 하지만 같은 thread/topic task 재사용과 planning/context는 hook 경로가 아닌 `codex-monitor` skill 쪽 책임이다.
- hooks와 skill을 같이 켜면 현재는 `codex-hook`, `codex-skill` runtimeSource가 분리되므로 task lineage도 별도로 보일 수 있다.
