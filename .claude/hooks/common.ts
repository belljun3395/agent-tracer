...
export function inferCommandLane(command: string): "rules" | "implementation" {
  const normalized = command.toLowerCase().trim();
  for (const pattern of RULES_PATTERNS) {
    if (normalized.startsWith(pattern) || normalized.includes(` ${pattern}`) || normalized.includes(`&&${pattern}`)) {
      return "rules";
    }
  }
