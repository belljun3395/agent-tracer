...
```
session_id              — unique session identifier
stop_reason             — "end_turn" | "max_tokens" | … (undocumented extension)
last_assistant_message  — final text of Claude's response (undocumented extension)
usage.input_tokens      — token counters (undocumented extension)
usage.output_tokens
...
POST /api/assistant-response
  taskId    [CACHE] from session cache
  sessionId [CACHE]
  messageId [GEN]   UUID
  ...
  metadata:
    ...
    inputTokens       [CC]  usage.input_tokens        (omitted if absent)
    outputTokens      [CC]  usage.output_tokens       (omitted if absent)
    cacheReadTokens   [CC]  usage.cache_read_input_tokens    (omitted if absent)
    cacheCreateTokens [CC]  usage.cache_creation_input_tokens (omitted if absent)
```
